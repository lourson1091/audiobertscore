...
class LocalOnlyEncoder(AudioNTT2022Encoder):
    def forward(self, x):
        x = self.features(x)       # (B, ch, mel, time)
        x = x.permute(0, 3, 2, 1)  # (B, time, mel, ch)
        B, T, D, C = x.shape
        x = x.reshape((B, T, C*D)) # (B, time, conv_d)
        return x

class FlexibleAudioEncoder(nn.Module):
    def __init__(self, n_mels, base_d=64, mlp_hidden_d=2048, conv_layers=2, mode='global'):

        d = 3072
        """
        Args:
            n_mels (int): メルスペクトログラムの次元数
            d (int): 最終的な特徴ベクトル次元
            base_d (int): 畳み込み層のチャンネル数
            mlp_hidden_d (int): MLP中間層の次元
            conv_layers (int): 畳み込み層の段数
            mode (str): 'local', 'global', 'concat'
        """
        super().__init__()
        assert mode in ['local', 'global', 'concat'], f"Unknown mode: {mode}"
        self.mode = mode
    ...
    def forward(self, x):
        ...
        if self.mode == 'local':
            return x  # conv_d 次元の局所特徴

        x_fc = self.fc(x)  # MLP通過 (B, time, d - conv_d)

        if self.mode == 'global':
            return x_fc

        elif self.mode == 'concat':
            return torch.cat([x, x_fc], dim=2)  # (B, time, d)
