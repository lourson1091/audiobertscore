    ...
    def get_intermediate_layers(self, x, length, n=1, scene=True):
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if len(self.blocks) - i <= n:
                ...
                if scene:
                    # scene embedding: average over time
                    length_mask = torch.arange(x.shape[1] - self.nprompt).to(x.device) < patch_length.unsqueeze(1)
                    avg = torch.sum(norm_x[:, self.nprompt:] * length_mask.unsqueeze(-1), dim=1) / (patch_length.unsqueeze(-1) + 1e-6)
                    output.append(avg)
                    ...
                    # timestamp embedding: return per block without concatenation
                    output.append(norm_x[:, self.nprompt:])  # shape: [B, T, D]

        if scene:
            return torch.cat(output, dim=-1)  # [B, D × n]
        else:
            return output  # List of [B, T, D]
