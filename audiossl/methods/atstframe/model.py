...
class FrameATST(nn.Module):
    ...
    def forward(self, x, length, return_time_features=False):
        """
        Forward pass for FrameATST.

        Args:
            x: Input tensor.
            length: Length of the input sequences.
            mask: Mask for the input.
            return_time_features: If True, return [T, D] features instead of aggregated embeddings.

        Returns:
            Tensor of shape [T, D] if return_time_features is True, otherwise [1, D].
        """
        if return_time_features:
            # Extract time-dependent features from the encoder
            time_features = self.student.encoder(x, length, return_features=True)
            return time_features  # [T, D]
        else:
            # Default behavior: return aggregated embeddings
            tea = self.teacher(x, length, False)
            stu = self.student(x, length, True)
