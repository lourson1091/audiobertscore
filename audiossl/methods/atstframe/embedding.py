...
# 各層の出力を保持したい場合のコード（scene=False, 層ごとに分ける）
def get_timestamp_embedding(audio, model):
    if len(audio.shape) == 2:
        audio = audio.unsqueeze(1)
    else:
    ...
    output_per_block = None  # List[Tensor] of shape [N_BLOCKS][B, T, emb_size]
    ...
    for i in range(num_chunks):
        ...
        if end > start:
            ...
            # 必要条件：この関数が block-wise に返す
            block_outputs = model.get_intermediate_layers(mel_chunk, len_chunk, n=N_BLOCKS, scene=False)

            if isinstance(block_outputs, list):  # ← ここが重要！
                if output_per_block is None:
                    output_per_block = [[] for _ in range(len(block_outputs))]

                for b_idx, block_out in enumerate(block_outputs):
                    output_per_block[b_idx].append(block_out)  # chunk 結合

            else:
                raise RuntimeError("get_intermediate_layers is returning concatenated output, not per-block")

    # 各ブロックの出力を時系列で連結した後に squeeze
    output_per_block = [
        torch.cat(block_outputs, dim=1).squeeze(0)  # [1, T, emb_size] → [T, emb_size]
        for block_outputs in output_per_block
    ]

    # timestamp 生成
    length = output_per_block[0].shape[1]
    timestamps = (torch.arange(length) * 40).float().unsqueeze(0).expand(mel.shape[0], -1)

    return output_per_block, timestamps