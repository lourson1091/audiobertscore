...
class ASTModel(nn.Module):
    ...
    #     for blk in self.v.blocks:
    #         x = blk(x)
    #         features_by_layer.append(x.clone())  # 各層後の出力を保存

    #     x = self.v.norm(x)
    #     features_by_layer.append(x.clone())  # LayerNorm後も保存（13層）
